...
import {
  ...
  UseUpdateAccountFormProps,
} from "./update-account-form.hook";

export interface UpdateAccountFormProps {
  onSuccess?: UseUpdateAccountFormProps["onSuccess"];
}

export const UpdateAccountForm = observer(
  ({ onSuccess }: UpdateAccountFormProps) => {
    const { fields, isSubmitting, submitForm } = useUpdateAccountForm({
      onSuccess,
    });
